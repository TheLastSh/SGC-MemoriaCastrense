    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <div class="flex items-start justify-between">
                <div>
                    <h1 class="text-2xl font-merriweather font-bold text-navy-900">{{ $hilo->titulo }}</h1>
        ...
        {{-- Respuestas --}}
        <h2 class="text-xl font-merriweather font-bold text-navy-900 mb-4">Respuestas ({{ $hilo->respuestas->count() }})</h2>

        <div class="space-y-4 mb-8">
            @forelse($hilo->respuestas as $respuesta)
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5" id="respuesta-{{ $respuesta->id }}">
            ...
            @empty
                <p class="text-gray-500 text-center py-8">No hay respuestas aún. Sé el primero en responder.</p>
            @endforelse
        </div>
            ...
            @if($hilo->status === 'abierto')
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <form action="{{ route('foro.responder', $hilo) }}" method="POST">
                        @csrf
                        <textarea name="contenido" rows="4" required minlength="3" placeholder="Escribe tu respuesta..." class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-gold-500"></textarea>
                        <button type="submit" class="mt-3 bg-gold-600 hover:bg-gold-500 text-navy-950 px-6 py-2 rounded-md text-sm font-bold transition-colors">Publicar Respuesta</button>
                    </form>
                </div>
            @else
                <p class="text-center text-gray-500 py-4">Este hilo está cerrado. No se pueden agregar nuevas respuestas.</p>
            @endif
        @else
            <p class="text-center text-gray-500 py-4"><a href="{{ route('login') }}" class="text-gold-600 hover:underline">Inicia sesión</a> para responder.</p>
        @endauth
    </div>
@endsection
