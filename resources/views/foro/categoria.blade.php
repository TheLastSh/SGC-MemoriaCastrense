...
@section('content')
    <div class="flex justify-between items-center mb-8" data-reveal>
        <div>
            ...
            <h1 class="text-heading text-navy-900 mt-1">{{ $categoria->nombre }}</h1>
        </div>
        @auth
            <a href="{{ route('foro.create-hilo', $categoria) }}" class="bg-gold-600/90 hover:bg-gold-500 active:scale-[0.97] text-navy-950 px-4 py-2 rounded-md text-sm font-bold shadow-sm transition-all">Nuevo Hilo</a>
        @else
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-2">
                        ...
                        @if($hilo->status === 'cerrado')
                            <span class="text-xs bg-red-50/80 text-red-600 px-2 py-0.5 rounded-full">Cerrado</span>
                        @endif
                    </div>
